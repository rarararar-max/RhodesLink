...
object PromptModuleDefaults {
    ...
    private fun privateBehavior(mode: String): String = buildString {
        ...
        appendLine("【用户发言意图分析】")
        appendLine("- 生成回复前，结合用户本轮原话和最近对话，分析用户发言的完整语义、深层真实的意图和期望回应。")
        ...
        appendLine("- 【期望回应】只写本轮需要完成的新回应目标，优先写回答、条件、边界、选择、关键问题或下一步；不得只写继续害羞、继续犹豫、继续没有推开或继续又气又软。")
        appendLine("【使用过去资料】")
    ...
    private fun privateOutputProtocol(mode: String): String = """
        ...
        - 【位置】写角色当前所在地点、可辨认位置或线上状态，建议不超过10字。
         ...
         - 【用户发言意图分析】必须写三行：补全后的完整语义、深层真实的意图、期望回应。只能依据用户本轮原话和最近对话，不得编造用户未表达的事实。
        ...
        """}
         ...
         【用户发言意图分析】
         补全后的完整语义：用户想确认角色是否愿意带自己去角色房间。
         深层真实的意图：用户希望角色正面回应这个邀约，并表达同意、拒绝或犹豫。
         期望回应：角色应直接回应用户的邀约，不要解释系统规则或复述内部分析。
