...
- 当前未被禁言的所有群成员，本轮都必须至少有一条自然台词；旁白不计入成员发言。本轮必须至少一条第三人称旁白。
...
【发言规则】
- 当前未被禁言的所有群成员本轮都必须至少发言1次；先确保每人自然接话一次，再按剧情需要安排额外发言。
- 每名成员可发言1~{{GROUP_SPEECH_MAX}}次，不要机械点名或轮流汇报。
...
- 线上模式彻底不允许旁白；线下/导演模式每轮必须至少一条旁白，用于动作衔接或氛围表达。
...
- 连续发言不限制，自然对话流
- 活跃度只影响额外发言次数；每位当前未被禁言成员仍必须至少发言1次。
