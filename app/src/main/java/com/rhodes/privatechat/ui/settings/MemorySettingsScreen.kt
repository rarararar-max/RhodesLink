...
private fun MemoryOverview(settings: SettingsRepository) {
            ...
            SettingsSwitchCard("统一记忆系统", "允许生成、索引和召回 Memory V2 记忆", settings.memoryV2Enabled) { settings.memoryV2Enabled = it }
...
private fun MemoryGeneration(settings: SettingsRepository) {
            ...
            SettingsSwitchCard("群聊记忆复制给成员", "将重要群聊记忆复制到成员的个人知识中", settings.groupMemoryCopyToMembersEnabled) { settings.groupMemoryCopyToMembersEnabled = it }
            ...
            SettingsSwitchCard("群聊长期沉淀", "允许群聊 L1 继续合并为 L2/L3", settings.groupMemoryPromotionEnabled) { settings.groupMemoryPromotionEnabled = it }
            ...
            SettingsParamSlider(settings, "memory_v2_promote_l1_threshold", "L1 合并为 L2 的阈值", 20, 5f..100f, "同一话题的短期记忆积累到多少条后合并为中期记忆。", step = 1f)
            SettingsParamSlider(settings, "memory_v2_promote_l2_threshold", "L2 合并为 L3 的阈值", 10, 3f..50f, "同一话题的中期记忆积累到多少条后合并为长期记忆。", step = 1f)
}
...
private fun MemoryInjection(settings: SettingsRepository) {
            ...
            SettingsParamSlider(settings, "private_group_context_count", "私聊最多读取几个相关群", 2, 0f..10f, "控制私聊自动参考的相关群数量。", step = 1f)

            SettingsSectionTitle("群聊允许读取")
            InjectionSwitch(settings, "group_chat", "GROUP_CHAT", "本群记忆")
            ...
            InjectionSwitch(settings, "group_chat", "MEMBER_PRIVATE_CHAT", "成员个人私聊记忆")
            SettingsParamSlider(settings, "group_member_memory_count", "群聊按需读取成员记忆数", 2, 0f..2f, "用户明确提到成员时，最多读取该成员多少条个人记忆。", step = 1f)
