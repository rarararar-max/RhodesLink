...
class WorldEventRepository(private val wrapper: DatabaseWrapper) {
    ...
    suspend fun insertWorldEvent(event: WorldEvent) = withContext(Dispatchers.Default) {
    ...
    suspend fun getRecentWorldEvents(limit: Int = 20): List<WorldEvent> = withContext(Dispatchers.Default) {
        ...
        db.worldEventsQueries.getRecentWorldEvents(now, limit.toLong()).executeAsList().map { row ->
            mapEvent(row.id, row.type, row.actorId, row.actorName, row.targetId, row.targetName, row.source, row.sourceId, row.content, row.createdAt, row.expiresAt, row.consumedBy)
        }
    }
    ...
    private fun mapEvent(
        ...
        expiresAt: Long,
        consumedBy: String
    ): WorldEvent = WorldEvent(id, type, actorId, actorName, targetId, targetName, source, sourceId, content, createdAt, expiresAt, consumedBy)
}
