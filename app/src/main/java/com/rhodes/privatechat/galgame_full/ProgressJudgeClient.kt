...
object ProgressJudgeClient {
    suspend fun judge(state: GameState, assets: ProjectAssets): Result<ProgressReport> {
        ...
        val system = "你是互动视觉小说的章节进度判定 Agent。只根据给定事实和对话判断，不创造事实，不修改状态。只返回 JSON：{\"status_description\":\"\",\"chapter_status\":\"in_progress|ready|completed|blocked\",\"completion\":false,\"unmet_conditions\":[],\"next_direction\":\"\",\"evidence\":[],\"chapter_summary\":\"\",\"key_facts\":[],\"relationship_notes\":[]}。章节内容：${chapter.contentDescription}；章节目标：${chapter.goal}；完成提示：${chapter.completionHint}."
        val user = "变量：${state.variables}；道具：${state.items}；事件：${state.events}；最近对话：${state.messages.takeLast(8).joinToString("\n") { "${it.speaker}：${it.text}" }}"
        requestLog = "$system\n\n$user"
